...
## 抽牌界面（半圆轮盘方案，参考截图）
> **[UPDATE] 设计变更说明**：
> 原“半圆轮盘”方案已升级为 **"Digital Stage / Spotlight"** 方案（沉浸式暗黑模式 + 聚光灯物理光效）。
> 详见 `docs/02-features/draw-page-overhaul.md`。以下描述仅作历史参考。
...
- 多档/套餐定价（暂不做）。
- 结果邮件模板可后置；高级推荐算法暂不做。
- **C 端用户系统**：不支持普通用户注册/登录，采用匿名/即用即走模式。
- **问卷数据持久化**：用户提交的问卷答案仅用于当次生成解读，不保存到数据库。
- **邮件/通知服务**：不发送任何邮件或通知（如解读结果、支付凭证等）。
