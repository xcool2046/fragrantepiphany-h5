            ...
            for (let r = 0; r < data.length; r++) {
                ...
                for (let i = 0; i < 3; i++) {
                    ...
                    if (!contentZh && !contentEn)
                        continue;
                    // Check existing
                    const existing = await AppDataSource.query(`SELECT id FROM "tarot_interpretations" WHERE "card_name" = $1 AND "category" = $2 AND "position" = $3`, [cardName, fileInfo.category, pos]);
                    if (existing && existing.length > 0) {
                        await AppDataSource.query(`UPDATE "tarot_interpretations" SET 
                      "recommendation_zh" = $1, "recommendation_en" = $2, 
                      "interpretation_zh" = $3, "interpretation_en" = $4
                     WHERE "id" = $5`, [sentenceZh, sentenceEn, contentZh, contentEn, existing[0].id]);
                    }
                    else {
                        await AppDataSource.query(`INSERT INTO "tarot_interpretations" 
                      ("card_name", "category", "position", "recommendation_zh", "recommendation_en", "interpretation_zh", "interpretation_en")
                     VALUES ($1, $2, $3, $4, $5, $6, $7)`, [cardName, fileInfo.category, pos, sentenceZh, sentenceEn, contentZh, contentEn]);
                    }
                }
                count++;
            }
