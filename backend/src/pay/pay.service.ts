...
function getPriceIdByCurrency(currency: string): string {
  const raw =
    process.env.STRIPE_PRICE_IDS_JSON_TEST && process.env.STRIPE_SECRET_KEY?.startsWith('sk_test_')
      ? process.env.STRIPE_PRICE_IDS_JSON_TEST
      : process.env.STRIPE_PRICE_IDS_JSON;

  if (!raw) {
    throw new Error('STRIPE_PRICE_IDS_JSON is required to map currency to price_id');
  }
  ...
  const priceId = mapping[currency.toLowerCase()];
  if (!priceId) {
    throw new Error(`No price_id configured for currency: ${currency}`);
  }
  return priceId;
}
...
export class PayService {
  ...
  async createSession(input: {
    ...
    const priceId = getPriceIdByCurrency(input.currency);
    ...
    try {
      await this.stripe.checkout.sessions.update(session.id, { metadata: session.metadata });
    } catch (err) {
    ...
    // 替换回调 URL 里的占位符
    const successUrl = session.success_url?.replace('{ORDER_ID}', order.id) ??
      `${publicBaseUrl}/pay/callback?status=success&order_id=${order.id}&session_id=${session.id}`;
    const cancelUrl = session.cancel_url?.replace('{ORDER_ID}', order.id) ??
      `${publicBaseUrl}/pay/callback?status=cancel&order_id=${order.id}`;

    try {
      await this.stripe.checkout.sessions.update(session.id, { success_url: successUrl, cancel_url: cancelUrl });
    } catch (err) {
  ...
  async handleWebhook(event: Stripe.Event) {
    ...
    if (event.type === 'checkout.session.completed') {
      ...
      // 校验金额与币种一致
      if (session.currency !== order.currency || (session.amount_total ?? session.amount_subtotal ?? 0) !== order.amount) {
      ...
      await this.orders.update(
        { id: orderId },
        {
          status: 'succeeded',
          stripe_session_id: session.id,
          payment_intent_id: (session.payment_intent as string) || order.payment_intent_id || undefined,
        },
      );
      return { ok: true };
    }
