...
let InterpretationService = class InterpretationService {
    ...
    async importMany(items) {
        ...
        for (const item of items) {
            ...
            if (item.sentence !== undefined)
                base[`sentence${prefix}`] = item.sentence;
    ...
    async findAll(page = 1, limit = tarot_1.DEFAULT_PAGE_SIZE, filters = {}) {
        ...
        if (filters.language) {
            ...
            qb.andWhere(`(i.sentence_${lang} IS NOT NULL OR i.interpretation_${lang} IS NOT NULL)`);
        }
        if (filters.keyword) {
            const kw = `%${filters.keyword.trim()}%`;
            qb.andWhere(`(i.card_name ILIKE :kw OR i.sentence_en ILIKE :kw OR i.sentence_zh ILIKE :kw OR i.interpretation_en ILIKE :kw OR i.interpretation_zh ILIKE :kw)`, { kw });
        }
