...
export default function Questions() {
  ...
  const save = async () => {
    ...
    try {
      if (editing) {
        await axios.patch(`/api/admin/questions/${editing.id}`, payload, { headers: tokenHeader })
      } else {
        await axios.post('/api/admin/questions', payload, { headers: tokenHeader })
      }
      setModalOpen(false)
      fetchData()
    } catch (e) {
  ...
  const toggleActive = async (q: Question) => {
    await axios.patch(`/api/admin/questions/${q.id}`, { active: !q.active }, { headers: tokenHeader })
    fetchData()
  }

  const adjustWeight = async (q: Question, delta: number) => {
    await axios.patch(`/api/admin/questions/${q.id}`, { weight: q.weight + delta }, { headers: tokenHeader })
    fetchData()
  }

  const remove = async (q: Question) => {
    if (!window.confirm('确认删除该问题？')) return
    await axios.delete(`/api/admin/questions/${q.id}`, { headers: tokenHeader })
    fetchData()
  }
